...
状态说明：✅ 已完成；🟡 部分完成 / 待补交付物；⬜ 未开始。

| 里程碑 | 板块 | 状态 | 当前成果 / 下一步 |
|--------|------|------|------------------|
| M1 | P0-2 产品选型对比矩阵 | ✅ | 已完成 C1901 / C1902 / C1903 / C2401 / 官方 DevKit 中英双语对比页，并加入首页与导航入口。 |
| M1 | P1-4 硬件资源下载区 | 🟡 | 已完成中英双语汇总页、图片与刷机入口；待发布 Datasheet、原理图、STEP、正式固件、版本号与校验值。 |
| M1 | P1-6 FAQ / 故障排查 | ✅ | 已完成中英双语 FAQ，覆盖刷机、启动、存储、USB、设备树、网络、GPIO、CAN、摄像头与 AI 软件。 |
| M1 | P1-7 Getting Started | 🟡 | 已有中英双语 `/quick-start` 学习路径；待补开箱清单、硬件接线、首次开机和第一个可验证 Demo，并统一最终路由。 |
| M1 | §4.1 Pinmux / 设备树 | 🟡 | 已完成中英双语通用流程；待补各载板 `.conf`、Pinmux/DTSI/DTB、Board ID/SKU、信号映射和验证矩阵。 |
| M1 | §4.1 40-pin 扩展头 | ✅ | 已完成 Jetson-IO 图形界面、命令行、GPIO 使用、载板兼容性与量产说明，中英双语已同步。 |
| M1+ | C1903 产品资料 | ✅ | 已新增 C1903 中英双语规格页、产品对比、资源入口与导航。 |
| M2 | P0-1 Benchmark 中心 | ⬜ | 先以一个 YOLO 模型跑通环境、延迟、FPS、显存、功耗与温度模板。 |
| M2 | P0-3 应用案例库 | ⬜ | 首篇优先选择 YOLO 端到端部署或 Ollama 本地问答。 |
| M2 | §4.1 相机 Sensor 驱动移植 | ⬜ | 需要选定已验证 Sensor、JetPack 和载板组合。 |
| M2 | §4.1 电源与性能管理 | ⬜ | 需覆盖 nvpmodel、jetson_clocks、温度、降频与各载板验证数据。 |
| M3 | 多媒体 / OTA / CAN / 内核安全 | ⬜ | 尚未建立对应专章。 |
| M3 | 配件兼容 / 备份克隆 / DeepStream / 一键部署 / 视频 | ⬜ | 尚未建立对应板块。 |

### 当前验收基线

- 中文与英文侧栏已加入产品选型、资源下载、载板适配开发、C1903 和 FAQ。
- 当前文档站可通过 `npm run docs:build` 完整构建。
- M1 在硬件资源正式文件和板级 BSP 资料交付前仍视为“部分完成”，不能仅以页面已建立作为完成标准。
...
**我们已覆盖（较强）：**
- 载板产品介绍（C1901 / C1902 / C1903 / C2401）
...
| ✅ P0 | 2. 产品选型对比矩阵 | 促成购买决策 | 低 | 已完成中英双语页面 |
| 🔴 P0 | 3. 应用案例库（端到端 Demo） | 证明产品能干活、转化力最强 | 高 | 需完整跑通+录屏 |
| 🟡 P1 | 4. 硬件资源下载区 | 工程师刚需（原理图/尺寸/pinout） | 低 | 汇总页已完成，正式资料待发布 |
| 🟠 P1 | 5. 配件与兼容性列表 | 减少售前售后咨询 | 中 | 需实测兼容性 |
| ✅ P1 | 6. FAQ / 故障排查中心 | 降低支持工单 | 低 | 已完成中英双语页面 |
| 🟡 P1 | 7. Getting Started 快速上手总入口 | 新手不迷路 | 低 | 学习路径已完成，开箱与首个 Demo 待补 |
...
英文站继续同步补齐。目前产品介绍、选型、资源、FAQ、Pinmux、40-pin、刷机和部分入门/进阶教程已有英文版；Ollama、ComfyUI、TensorRT 等进阶教程及后续 Benchmark / Applications 仍需同步。
